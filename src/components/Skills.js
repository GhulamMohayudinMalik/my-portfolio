...
// Optimized SkillCard component with reduced animations for mobile
const SkillCard = React.memo(({ skill, index, isMobile }) => {
  const IconComponent = skill.icon;
  
  return (
    <div className={`group relative bg-gray-800/40 border border-gray-700/40 rounded-2xl p-4 hover:border-green-400/60 transition-all duration-300 ${
      !isMobile ? 'hover:transform hover:scale-105 hover:shadow-lg hover:shadow-green-400/20' : ''
    }`}>
      {!isMobile && (
      ...
      <div className="relative z-10 flex flex-col items-center text-center space-y-3">
        <div 
          className={`w-14 h-14 rounded-xl flex items-center justify-center shadow-lg border-2 ${
            !isMobile ? 'group-hover:rotate-6 transition-transform duration-300' : ''
          }`}
          style={{ 
        ...
        >
          <IconComponent 
            ...
            className={!isMobile ? "group-hover:scale-110 transition-transform duration-300" : ""}
          />
        </div>
              ...
              {/* Header */}
                ...
                >
                  ...
                  <motion.p
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    transition={{ duration: isMobile ? 0.5 : 1, delay: 0.5 }}
                ...
                >
                  <div className={`rounded-3xl bg-gray-900/60 p-8 border-2 border-gray-700/30 hover:border-green-400/30 transition-all duration-300 group relative overflow-hidden ${
                    ...
                    <div className="relative z-10">
                      <div className={`w-20 h-20 bg-gradient-to-r from-green-400 to-emerald-500 rounded-2xl flex items-center justify-center text-4xl mb-8 shadow-lg shadow-green-400/30 ${
                        !isMobile ? 'group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300' : ''
                      }`}>
                        🌐
                      </div>
